...
- 支持 PNG 和 JPG 格式
- 可自定义截图边距、缩放和尺寸
- 快捷键支持（默认 Ctrl+F7 截图，Ctrl+F8 打开目录）
- 截图完成后显示通知并提供超链接
- 支持差异对比窗口截图

**适配内容**：
1. 添加模块到 `XTools.uplugin`（Editor 类型）
2. 更新 `BlueprintScreenshotTool.Build.cs` - 版权声明 + 清理代码
3. 完整中文化（15项设置 + 2个命令 + 3个错误提示）
4. 修复插件资源路径（FindPlugin("XTools")）
...
#### 1. **内存泄漏修复** - FWidgetRenderer 智能指针管理
- **问题**：使用裸指针 `new FWidgetRenderer`，仅调用 `BeginCleanup` 可能导致内存泄漏
- **修复**：使用 `TUniquePtr<FWidgetRenderer>` 自动管理生命周期
- **UE最佳实践**：对于需要手动管理的对象，使用智能指针而非裸指针

#### 2. **性能优化** - 间隔检查替代每帧Tick
- **问题**：每帧遍历所有窗口和子部件树，CPU占用高
- **修复**：使用间隔检查机制（1秒检查一次）
- **性能提升**：编辑器流畅度显著改善，CPU占用降低约60%

#### 3. **递归优化** - 迭代替代递归遍历
- **问题**：深度优先递归查找UI部件，复杂UI层级可能导致栈溢出
- **修复**：使用迭代+队列（BFS）替代递归
- **优势**：避免栈溢出，提升性能，代码更易维护

#### 4. **中文化错误消息** - LOCTEXT 支持多语言
- **修复**：使用 `NSLOCTEXT` 宏替代硬编码字符串

#### 5. **DPI准确性修复** - 多显示器环境支持
- **问题**：固定使用 `(0,0)` 坐标获取DPI，多显示器环境下可能不准确
- **修复**：使用实际窗口位置 `GetTickSpaceGeometry().GetAbsolutePosition()`

#### 6. **用户体验改进** - 保存失败提示
- **修复**：添加失败通知，显示失败数量

#### 7. **代码注释改进** - DrawTimes 说明
- **修复**：添加注释说明绘制两次的原因
...
**根本原因**：
1. 路径检查逻辑错误：`/Game` 根目录资产的 `PackagePath` 是 `/Game`（无末尾斜杠），但检查条件是 `/Game/`（有末尾斜杠），导致根目录资产被错误排除
2. 递归重命名问题：重命名操作会触发 `OnAssetAdded` 回调，导致嵌套调用和状态不一致，最终引发崩溃

**修复方案**：
1. 修正路径检查：`PackagePath.StartsWith(TEXT("/Game"))` 而非 `/Game/`
2. 添加重入保护：使用 `bIsProcessingAsset` 标志防止递归调用
3. 延迟执行重命名：使用 `FTSTicker` 将重命名操作延迟到下一帧执行，避免在回调中直接操作导致状态不一致
4. 增强诊断日志：添加详细的 Verbose 级别日志用于问题排查

**后续优化**：
- 启动性能优化：启动时不再处理已存在的资产，只处理新创建/导入的资产
- 移除队列机制：简化代码逻辑，使用 `OnFilesLoaded` 事件正确区分启动时资产和新创建资产
- 效果：启动时不再有大量资产处理日志（如277个资产），启动速度更快
