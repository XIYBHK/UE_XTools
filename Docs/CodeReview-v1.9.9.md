...
| M19 | `Source/XTools/XTools.Build.cs` | Runtime 主模块在编辑器目标下链接 UnrealEd/Kismet/BlueprintGraph/KismetCompiler，违背 AGENTS.md 自身红线（有双重守卫暂不致打包失败） | **部分处理**：移除零使用的 Kismet/GraphEditor/EditorStyle/EditorWidgets/AppFramework/ToolWidgets 并修正失实注释，保留项均经头文件归属+导出符号取证（UnrealEd/BlueprintGraph/KismetCompiler/AssetRegistry/ComponentTimelineUncooked），Editor Development 与 Game Development/Shipping 三目标编译通过；Runtime/Editor 拆分经复审否决——CleanupTool 为运行时可见 API+编辑器门控实现模式、无打包缺陷，拆分会引入打包语义变化，Kismet 移除需过 5.4-5.8 CI 确认头文件归属未迁移 |
...
- **P2（部分处理）**：M10–M13 生命周期弱指针化、M21–M24 行为一致性批次、M25 贝塞尔性能债务（已补警示）、FormationMovement 制动带死锁；M17/M18 对象池维护层已完成不可达子系统清理，公共维护类架构决策仍开放；M19 已完成依赖卫生，主模块拆分经复审否决。
...
- 仍开放：M18 公共维护类（FActorStateResetter/FActorPoolMemoryOptimizer）的接线、废弃或移除决策；卡墙超时/失败事件及其他低优先级规范与性能债务。
