...
public:
	...
	UPROPERTY(EditAnywhere, config, Category = "Cache", meta = (DisplayName = "写入图表节点尺寸", ToolTip = "将缓存的节点宽高写入 UEdGraphNode 的 NodeWidth 和 NodeHeight"))
	...
	UPROPERTY(EditAnywhere, config, Category = "Misc|Experimental", meta = (DisplayName = "立即显示图表遮罩", ToolTip = "开始缓存蓝图节点尺寸时立即显示遮罩，避免视口闪烁"))
