...
public:
	...
	UPROPERTY(EditAnywhere, config, Category = General, meta = (DisplayName = "晃动检测时间窗口", ClampMin = "0.1", UIMin = "0.1", ToolTip = "检测晃动移动的时间窗口(秒),在指定时间内晃动达到次数时触发断开连接"))
	...
	UPROPERTY(EditAnywhere, config, Category = FormattingOptions, meta = (DisplayName = "多引脚时禁用螺旋", ToolTip = "如果有多个链接的引脚则禁用螺旋排列"))
	...
	UPROPERTY(EditAnywhere, config, Category = FormattingOptions, meta = (EditCondition = "bDisableHelixingWithMultiplePins", DisplayName = "仅统计参数引脚", ToolTip = "计算禁用螺旋的引脚数量时，仅统计已连接的参数引脚"))
