...
void SBAVariableSelectorMenu::Construct(const FArguments& InArgs)
{
	ChildSlot
	[
		SNew(SBAFilteredList<TSharedPtr<FVariableSelectorStruct>>)
		.InitListItems(this, &SBAVariableSelectorMenu::InitListItems)
		.OnGenerateRow(this, &SBAVariableSelectorMenu::CreateItemWidget)
		.OnSelectItem(this, &SBAVariableSelectorMenu::SelectItem)
		.OnMarkActiveSuggestion(this, &SBAVariableSelectorMenu::MarkActiveSuggestion)
...
void SBAVariableSelectorMenu::InitListItems(TArray<TSharedPtr<FVariableSelectorStruct>>& Items)
{
...
TSharedRef<ITableRow> SBAVariableSelectorMenu::CreateItemWidget(TSharedPtr<FVariableSelectorStruct> Item, const TSharedRef<STableViewBase>& OwnerTable) const
{
...
void SBAVariableSelectorMenu::SelectItem(TSharedPtr<FVariableSelectorStruct> Item)
{
	FBlueprintEditor* BPEditor = FBAMiscUtils::GetEditorFromActiveTabCasted<UBlueprint, FBlueprintEditor>();
	if (!BPEditor)
	{
...
void SBAVariableSelectorMenu::MarkActiveSuggestion(TSharedPtr<FVariableSelectorStruct> Item)
{
	SelectItem(Item);
}

bool SBAVariableSelectorMenu::ShouldSkipProperty(BA_PROPERTY* Property)
{
#if ENGINE_MINOR_VERSION >= 25 || ENGINE_MAJOR_VERSION >= 5
