...
void SBAGoToSymbolMenu::Construct(const FArguments& InArgs)
{
	ChildSlot
	[
		SNew(SBAFilteredList<TSharedPtr<FGoToSymbolStruct>>)
		.InitListItems(this, &SBAGoToSymbolMenu::InitListItems)
		.OnGenerateRow(this, &SBAGoToSymbolMenu::CreateItemWidget)
		.OnSelectItem(this, &SBAGoToSymbolMenu::SelectItem)
		.WidgetSize(GetWidgetSize())
		.MenuTitle(FString("Go To Symbol"))
	];
}

void SBAGoToSymbolMenu::InitListItems(TArray<TSharedPtr<FGoToSymbolStruct>>& Items)
{
...
TSharedRef<ITableRow> SBAGoToSymbolMenu::CreateItemWidget(TSharedPtr<FGoToSymbolStruct> Item, const TSharedRef<STableViewBase>& OwnerTable) const
{
	...
	{
		ItemDetails += " | ";
		ItemDetails += FBAUtils::GetGraphDisplayName(Item->Graph);
	}
...
void SBAGoToSymbolMenu::SelectItem(TSharedPtr<FGoToSymbolStruct> Item)
{
