...
- **蓝图 AI 导出**: 总目录/资产入口按图提供 ReadPack v2 伪代码、内容清单与按需证据，`05_Query.py` 离线读取预算受限的入口子图；目录按完整资产路径摘要隔离，`90_Full/` 仅保留完整 JSON 归档。单次采集 Snapshot 后由 `XBlueprintReadPack::Build` 纯消费 JSON；重导时在文件事务中退役旧 `.ai.md`、人读 `.md`，不再维护独立展示器。设计兼容 UE 5.3–5.8，不依赖 5.8 MCP。格式及回归入口见 [蓝图 AI 逻辑导出](../../Docs/开发文档/蓝图AI逻辑导出.md)
