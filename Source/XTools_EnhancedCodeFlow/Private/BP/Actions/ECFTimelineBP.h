...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FOnECFTimelineBPEvent, float, Value, float, Time, bool, bStopped, FName, EventName);
...
public:
	...
	static UECFTimelineBP* ECFTimeline(const UObject* WorldContextObject, float StartValue, float StopValue, float Time, FECFActionSettings Settings, FECFHandleBP& Handle, EECFBlendFunc BlendFunc, float BlendExp, float PlayRate, EECFPlayDirection PlayDirection, const TArray<FECFTimelineEvent>& Events);
};
