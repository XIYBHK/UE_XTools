...
public:
  ...
  static TArray<FVector> GenerateFromTextureAutoWithPoisson(
      UTexture2D *Texture, int32 MaxSampleSize, float MinRadius,
   ...
   * @param Texture 纹理引用
   * @param MaxSampleSize
   * 最大采样尺寸（纹理会被降采样到此尺寸以下，控制最大点数量）
   * @param Spacing 像素步长（采样间隔，值越大点越稀疏）
   * @param PixelThreshold 像素采样阈值 (0-1)，只有亮度/Alpha
   * 高于此值的像素会被采样
   * @param TextureScale 纹理缩放（影响生成点位的物理尺寸）
   * @return 基于像素的点位数组（局部坐标，居中）
   */
  static TArray<FVector> GenerateFromTexture(UTexture2D *Texture,
                                             int32 MaxSampleSize, float Spacing,
  ...
  GenerateFromTextureWithPoisson(UTexture2D *Texture, int32 MaxSampleSize,
...
private:
   ...
   * @param bInvert 是否反转采样值 (用于黑底白图 vs 白底黑图的自动适配)
   * @return 采样值 (0-1)
   */
   ...
   */
  static float
  CalculatePixelSamplingValueByChannel(const FLinearColor &Color,
                                       ETextureSamplingChannel Channel);
   ...
   * @return 计算出的半径
   */
  static float CalculateRadiusFromDensity(float Density, float MinRadius,
                                          float MaxRadius);
   ...
   */
  static TArray<FVector> GenerateFromTextureSource(UTexture2D *Texture,
                                                   int32 MaxSampleSize,
                                                   float Spacing,
                                                   float PixelThreshold,
                                                   float TextureScale);
   ...
   */
  static TArray<FVector>
  GenerateFromTextureSourceWithPoisson(UTexture2D *Texture, int32 MaxSampleSize,
                                       float MinRadius, float MaxRadius,
                                       float PixelThreshold, float TextureScale,
                                       int32 MaxAttempts);
   ...
   */
  static TArray<FVector> GenerateFromTexturePlatformData(UTexture2D *Texture,
                                                         int32 MaxSampleSize,
                                                         float Spacing,
                                                         float PixelThreshold,
                                                         float TextureScale);
  ...
  static float GetTextureDensityAtCoordinatePlatform(
      ...
      int32 OriginalHeight, uint32 BytesPerPixel, bool bInvert = false);
   ...
   */
  static bool ValidateAndLogPlatformTextureFormat(EPixelFormat PixelFormat,
  ...
  CreateTemporaryMaterialForTexture(UTexture2D *Texture, UWorld *World);
   ...
   * @param TargetWorldSize 目标世界尺寸 (Width, Height)
  ...
  static TArray<FVector> GeneratePointsFromRenderTargetWithPoisson(
      UTextureRenderTarget2D *RenderTarget, int32 MaxSampleSize,
      float MinRadius, float MaxRadius, float PixelThreshold,
      FVector2D TargetWorldSize, ETextureSamplingChannel SamplingChannel,
      int32 MaxAttempts);
};
