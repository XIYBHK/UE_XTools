...
void UPointSamplingLibrary::AnalyzeSamplingStats(
	...
	FVector& OutCentroid)
{
	...
	{
		UE_LOG(LogPointSampling, Warning, TEXT("[采样统计] 点集较大 (%d 点)，距离计算复杂度为 O(N²)，可能影响性能"),
			NumPoints);
	}
	...
	float Tolerance)
{
	...
	if (!ensure(ExpectedMinDistance > 0.0f && Tolerance >= 0.0f && Tolerance <= 1.0f))
	{
		UE_LOG(LogPointSampling, Error, TEXT("[泊松验证] 无效参数: ExpectedMinDistance=%.2f, Tolerance=%.2f"),
	...
	if (NumPoints > MaxReasonablePoints)
	{
		UE_LOG(LogPointSampling, Warning, TEXT("[泊松验证] 点集较大 (%d 点)，验证复杂度为 O(N²)"),
			NumPoints);
	}
		...
		for (int32 j = i + 1; j < NumPoints; ++j)
		{
			...
			if (Distance < MinAllowedDistance)
			{
				UE_LOG(LogPointSampling, Warning,
					TEXT("泊松采样验证失败：点 %d 和 %d 之间的距离 %.2f 小于最小允许距离 %.2f"),
	...
	switch (Mode)
	{
		...
		case EPointSamplingMode::Circle:
			return UFormationSamplingLibrary::GenerateCircle(
				...
				false, // bIs3D
				static_cast<ECircleDistributionMode>(FMath::Clamp(Param3, 0, 2)), // DistributionMode
		...
		case EPointSamplingMode::EchelonRight:
		{
			const int32 Direction = (Mode == EPointSamplingMode::EchelonLeft) ? -1 :
								   (Mode == EPointSamplingMode::EchelonRight) ? 1 : Param3;
...
float UPointSamplingLibrary::CalculateDistributionUniformity(const TArray<FVector>& Points)
{
	if (Points.Num() < 3)
	{
