 ...
 */
UCLASS()
class POINTSAMPLING_API UPointSamplingLibrary
...
public:
  ...
  static TArray<FVector2D> GeneratePoissonPoints2D(float Width, float Height,
                                                   float Radius,
                                                   int32 MaxAttempts = 30);

  UFUNCTION(BlueprintCallable, Category = "Point Sampling|Poisson",
            meta = (DisplayName = "泊松采样 3D"))
  static TArray<FVector> GeneratePoissonPoints3D(float Width, float Height,
  ...
  static TArray<FVector> GeneratePoissonPointsInBox(
      ...
      int32 TargetPointCount = 0, float JitterStrength = 0.0f,
      bool bUseCache = true);
  ...
  static TArray<FVector> GeneratePoissonPointsInBoxByVector(
      FVector BoxExtent, FTransform Transform, float Radius = 50.0f,
      int32 MaxAttempts = 30,
      EPoissonCoordinateSpace CoordinateSpace = EPoissonCoordinateSpace::Local,
      int32 TargetPointCount = 0, float JitterStrength = 0.0f,
      bool bUseCache = true);
  ...
  static TArray<FVector> GeneratePoissonPointsInBoxByVectorFromStream(
      const FRandomStream &RandomStream, FVector BoxExtent,
      FTransform Transform, float Radius = 50.0f, int32 MaxAttempts = 30,
      EPoissonCoordinateSpace CoordinateSpace = EPoissonCoordinateSpace::Local,
      int32 TargetPointCount = 0, float JitterStrength = 0.0f);
  ...
  static void AnalyzeSamplingStats(const TArray<FVector> &Points,
                                   ...
                                   float &OutAvgDistance, FVector &OutCentroid);
   ...
   */
  UFUNCTION(
      BlueprintCallable, Category = "XTools|点采样|通用阵型",
  ...
  static TArray<FVector> GenerateFormation(
      ...
      EPoissonCoordinateSpace CoordinateSpace = EPoissonCoordinateSpace::Local,
      float Spacing = 100.0f, float JitterStrength = 0.0f, int32 RandomSeed = 0,
      // 阵型特定参数
