...
UCLASS(meta=(Keywords="DebugPrint Print String 调试 打印 多值 dayin"))
class BLUEPRINTEXTENSIONS_API UK2Node_DebugPrint : public UK2Node_EditablePinBase, public IK2Node_AddPinInterface
{
    GENERATED_BODY()
public:
    UK2Node_DebugPrint();
    ...
    UPROPERTY(EditAnywhere, EditFixedSize, Category="调试打印", meta=(DisplayName="自定义名称", ToolTip="按输入顺序设置名称；留空时自动使用来源名称。通过节点上的按钮添加和移除输入。"))
...
private:
    ...
    void NotifyChanged();
    void RefreshLabels();
    UEdGraphPin* AddValuePin(const FEdGraphPinType& Type);
};
