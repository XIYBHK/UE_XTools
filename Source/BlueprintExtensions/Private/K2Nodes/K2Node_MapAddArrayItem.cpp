...
FText UK2Node_MapAddArrayItem::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
    return LOCTEXT("NodeTitle", "Map添加数组元素");
}

FText UK2Node_MapAddArrayItem::GetCompactNodeTitle() const
{
    return LOCTEXT("CompactNodeTitle", "添加元素");
}

FText UK2Node_MapAddArrayItem::GetTooltipText() const
{
    return LOCTEXT("TooltipText", "向Map中结构体值的数组字段添加元素");
}

FText UK2Node_MapAddArrayItem::GetMenuCategory() const
{
    return LOCTEXT("MenuCategory", "XTools|Blueprint Extensions|Map");;
}

FSlateIcon UK2Node_MapAddArrayItem::GetIconAndTint(FLinearColor& OutColor) const
{
    static FSlateIcon Icon("EditorStyle", "GraphEditor.MakeMap_16x");
    return Icon;
}

TSharedPtr<SWidget> UK2Node_MapAddArrayItem::CreateNodeImage() const
{
...
class FKCHandler_MapAddArrayItem : public FNodeHandlingFunctor
{

public:

	FKCHandler_MapAddArrayItem(FKismetCompilerContext& InCompilerContext) 
    ...
    virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
	    ...
	    UK2Node_MapAddArrayItem* MapNode = CastChecked<UK2Node_MapAddArrayItem>(Node);
    ...
    virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
    {
	    ...
	    if (!AddArrayItemFunc)
	    {
	        Context.MessageLog.Error(*FString::Printf(TEXT("映射节点 @@ 编译失败：在 UMapExtensionsLibrary 中未找到函数 %s。"), TEXT("Map_AddArrayItem")), Node);
	        return;
	    }
...
class FNodeHandlingFunctor* UK2Node_MapAddArrayItem::CreateNodeHandler(class FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_MapAddArrayItem(CompilerContext);
}
...
void UK2Node_MapAddArrayItem::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
    K2NodeHelpers::RegisterNode<UK2Node_MapAddArrayItem>(ActionRegistrar);
}

void UK2Node_MapAddArrayItem::PostReconstructNode()
{
...
void UK2Node_MapAddArrayItem::AllocateDefaultPins()
{
    // 创建执行引脚
...
void UK2Node_MapAddArrayItem::PinDefaultValueChanged(UEdGraphPin* Pin)
{
    Super::PinDefaultValueChanged(Pin);
    PropagatePinType();
}

void UK2Node_MapAddArrayItem::ReconstructNode()
{
    Super::ReconstructNode();
    PropagatePinType();
}

void UK2Node_MapAddArrayItem::PinConnectionListChanged(UEdGraphPin* Pin)
{
...
bool UK2Node_MapAddArrayItem::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
    // 先检查参数有效性
    if (!MyPin || !OtherPin)
    {
...
const FName UK2Node_MapAddArrayItem::InputMapPinName(TEXT("MapPin"));
UEdGraphPin* UK2Node_MapAddArrayItem::GetInputMapPin() const
{
	return FindPin(InputMapPinName);
}

const FName UK2Node_MapAddArrayItem::InputKeyPinName(TEXT("KeyPin"));
UEdGraphPin* UK2Node_MapAddArrayItem::GetInputKeyPin() const
{
	return FindPin(InputKeyPinName);
}

const FName UK2Node_MapAddArrayItem::InputItemPinName(TEXT("ItemPin"));
UEdGraphPin* UK2Node_MapAddArrayItem::GetInputItemPin() const
{
...
void UK2Node_MapAddArrayItem::PropagatePinType()
{
    UEdGraphPin* MapPin = GetInputMapPin();
