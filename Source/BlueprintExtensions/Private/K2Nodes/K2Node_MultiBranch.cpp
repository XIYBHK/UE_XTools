...
UK2Node_MultiBranch::UK2Node_MultiBranch(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	NodeContextMenuSectionLabel = LOCTEXT("MultiBranch", "多分支");
...
{
	return LOCTEXT("MultiBranchStatement_Tooltip", "多分支语句\n类似 if-elseif-else，按顺序检查条件，执行第一个为 true 的分支\n右键可添加/删除条件分支");
}
...
{
	return LOCTEXT("MultiBranch", "多分支");
}
