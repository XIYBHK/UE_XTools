...
void UK2Node_ForLoopWithDelayReverse::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	// 编译时检查：确保必要引脚有效
	if (!GetExecPin() || !GetFirstIndexPin() || !GetLastIndexPin() || !GetDelayPin())
	{
		CompilerContext.MessageLog.Error(*LOCTEXT("MissingPins", "@@ 节点引脚不完整").ToString(), this);
		return;
	}
	...
	CompilerContext.GetSchema()->TryCreateConnection(LoopCounterBreak->GetValuePin(), BreakValue->GetReturnValuePin());
	...
	{
		CompilerContext.MovePinLinksToIntermediate(*BreakPin, *LoopCounterBreak->GetExecPin());
	}
